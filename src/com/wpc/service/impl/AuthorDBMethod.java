...
public class AuthorDBMethod extends PubCommonDAOImpl{
	...
	public Author save(Author entity) throws Exception {
		if (entity != null) {
			if (Utils.isEmpty(entity.getId())) {
				entity.setId(FrameworkHelper.getNewId("Author", idName, initId));
			}
	...
	public boolean delete(Author entity) throws Exception {
		if(null == entity){
			return false;
		}
		int rs = this.deleteSQL("FROM Author t WHERE t.id = ?", entity.getId());
	...
	public boolean deleteByIds(String[] ids) throws Exception {
		if (Utils.notEmpty(ids)) {
			...
			int rs = this.deleteSQL("FROM Author t WHERE t.id in("+idStr.substring(1)+")");
			return rs > 0 ? true : false;
		}
		return false;
	}
	...
	public List<Author> findByIds(String[] idArray) throws Exception {
		...
		String hql = "FROM Author t WHERE t.id in("+sb.substring(1)+") " ;
		List<Author> list = getQueryList(hql);
	...
	public Author findById(String id) throws Exception {
		if (Utils.notEmpty(id)) {
			List<Author> list = getQueryList("FROM Author t WHERE t.id = ?", id);
	...
	public List<Author> findByPage(Page page, SearchContainer searchContainer) {
		if(searchContainer == null){
			searchContainer = new SearchContainer();
			searchContainer.orderByDesc("t.id");
		}
		StringBuilder hql = new StringBuilder("FROM Author t ");
