 ...
 *
 */
public class AuthorController extends MultiActionController {
	Gson gson = new Gson();
	...
	public ModelAndView loadData(HttpServletRequest request,
		...
		if(dtRequest.isHasGroupAction()){
			...
			if(Utils.notEmpty(dtRequest.getSelectItem())){
				...
				if("delete".equals(action)){
					ServiceLocator.getAuthorService().deleteByIds(dtRequest.getSelectItem());
				}
			}
		}
		//过滤条件
		...
		List<Author> list = ServiceLocator.getAuthorService().findByPage(page, searchContainer);
		...
		if(Utils.notEmpty(list)){
			for(Author entity : list){
				tableData.addDataItem(
						...
						entity.getName(),
						entity.getIntros(),
						"<a href='../../../author.sp?act=addOrEdit&id="+entity.getId()+"' data-toggle='modal' data-target='#ajax'  class='btn btn-xs default'><i class='fa fa-search'></i>编辑</a>"
						);
			}
		}
	...
	public ModelAndView addOrEdit(HttpServletRequest request,
		...
		Author entity = ServiceLocator.getAuthorService().findById(id);
		ModelAndView modelAndView = new ModelAndView("admin/template/admin/author_add_edit.jsp");
		modelAndView.addObject("author", entity==null?new Author():entity);	
	...
	public ModelAndView save(HttpServletRequest request,
		...
		try {
			if(isNew){
				entity = ServiceLocator.getAuthorService().save(entity);
			}else{
				entity = ServiceLocator.getAuthorService().update(entity);
			}
			callResult.setRet(0);
